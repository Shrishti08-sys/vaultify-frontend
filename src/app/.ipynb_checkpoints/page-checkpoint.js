...
export default function Home() {
  ...
  useEffect(() => {
    fetch("https://vaultify-rtxr.onrender.com/api/subscriptions")
      .then((res) => res.json())
      .then((result) => setData(result))
      .catch((err) => console.error(err));
  }, []);
  const chartData = Object.keys(data.summary.categories).map((key) => ({
    name: key,
  ...
  return (
    <main className="min-h-screen bg-[#F7F9FC] p-8 font-sans text-gray-900 relative">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        {/* Upload Box Section for the User */}
            ...
            {data.subscriptions.map((sub) => (
              ...
              >
                ...
                <div className="text-right flex flex-col items-end gap-3">
                  ...
                  >
                    {sub.action_plan.action_type === "cancel"
                      ? "Cancel Service"
                      : "Renegotiate"}
                  </button>
                </div>
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100 space-y-4">
            ...
            <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl space-y-2">
              ...
              <p className="text-gray-800 font-medium leading-relaxed">
                {selectedSub.action_plan.guidance}
              </p>
            </div>
            ...
            <div className="pt-2 flex justify-end">
              <button
                onClick={() => setSelectedSub(null)}
                className="bg-gray-900 text-white px-5 py-2 rounded-lg text-sm font-bold hover:bg-gray-800 transition-all"
              >
                Close
              </button>
            </div>
