...
export default function Home() {
  ...
  const chartData = data.summary?.categories ? Object.keys(data.summary.categories).map(key => ({
    name: key,
    ...
    <main className="min-h-screen bg-[#F7F9FC] p-8 font-sans text-gray-900 relative">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        <header className="bg-white p-8 rounded-2xl shadow-sm border border-gray-200 flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-extrabold tracking-tight">Leak Detector</h1>
            <p className="text-gray-500 mt-2">Autonomous Recurring Payment & Subscription Audit</p>
          </div>
          <div className="text-right">
            <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Total Monthly Leak</p>
            <p className="text-5xl font-black text-rose-600 tracking-tighter">${data.summary.total_monthly_leak_amount}</p>
          </div>
        </header>

        <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
          <h2 className="text-lg font-bold mb-2">Upload Bank Statement</h2>
          <p className="text-sm text-gray-500 mb-4">Upload a PDF or CSV statement to scan for hidden subscriptions and price hikes.</p>
          ...
          <label className="border-2 border-dashed border-gray-300 rounded-xl p-6 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-500 transition-colors bg-gray-50/50">
            <input type="file" className="hidden" onChange={handleFileUpload} accept=".pdf,.csv,.txt" />
            ...
            {isScanning ? (
              ...
              <div className="text-emerald-600 font-semibold text-center">
                ✅ Statement successfully analyzed! Displaying detected leaks below.
              </div>
            ) : (
              <div className="text-center">
                <p className="text-sm font-semibold text-gray-700">Click to upload statement file</p>
                <p className="text-xs text-gray-400 mt-1">PDF, CSV or TXT up to 10MB</p>
              </div>
            )}
          </label>
        </section>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
            ...
            <div className="h-48 w-full">
              {chartData.length > 0 ? (
                ...
                <div className="h-full flex items-center justify-center text-sm text-gray-400">No category data available</div>
              )}
            </div>
          ...
          <section className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 flex flex-col justify-center items-center text-center">
            ...
            <p className="text-sm text-gray-500 mb-6">100 = Critical Financial Drain</p>
            
            <div className="relative w-32 h-32 flex items-center justify-center bg-rose-50 rounded-full border-8 border-rose-100">
              <span className="text-4xl font-black text-rose-600">{data.summary.leak_score}</span>
            </div>
          </section>
          ...
          <div className="grid gap-4">
            {data.subscriptions && data.subscriptions.length > 0 ? (
              data.subscriptions.map((sub) => (
                <div key={sub.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-200 flex justify-between items-center transition-all hover:shadow-md">
                  
                  <div>
                    <h3 className="text-lg font-bold">{sub.merchant_name}</h3>
                    <p className="text-sm text-gray-500 font-medium mt-1">{sub.category}</p>
                  ...
                  <div className="text-right flex flex-col items-end gap-3">
                    <p className="text-2xl font-bold">${sub.current_monthly_price}</p>
                    ...
                    >
                      ...
                      {sub.action_plan?.action_type === 'cancel_or_downgrade' ? 'Cancel Service' : 'Renegotiate'}
                    </button>
                  </div>
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-gray-100 space-y-4">
            <div className="flex justify-between items-center">
              <h3 className="text-xl font-bold">Action Plan: {selectedSub.merchant_name}</h3>
              <button 
            ...
            <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl space-y-2">
              <p className="text-xs font-bold text-indigo-600 uppercase tracking-wider">Recommended Action</p>
              <p className="text-gray-800 font-medium leading-relaxed">{selectedSub.action_plan?.guidance || "No guidance generated."}</p>
            </div>
